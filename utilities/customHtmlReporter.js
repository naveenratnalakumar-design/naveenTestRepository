  ...
  onEnd() {
    ...
    if (fs.existsSync(indexPath)) {
      ...
      if (platform === 'win32') {
        ...
        // Checks for Linux or other unix-based platforms
        command = `xdg-open "${indexPath}"`;
      }
      exec(command, (error) => {
        if (error) {
          console.error('Failed to open the report automatically:', error);
        }
      });
    }
  }
...
generateScenarioPage(scenario) {
  ...
  if (scenario.screenshot && fs.existsSync(scenario.screenshot)) {
    ...
    screenshotTag = `<h3>📸 Screenshot</h3><img src="${path.basename(destScreenshot)}?t=${timestamp}" width="600" />`;
  }
  ...
  if (scenario.video && fs.existsSync(scenario.video)) {
    ...
    videoTag = `<h3>🎥 Video</h3><video src="${path.basename(destVideo)}?t=${timestamp}" width="600" controls></video>`;
  }
  ...
  const html = `
    <html>
      ...
      <body>
        <h2>${scenario.name}</h2>
        <a href="index.html?t=${timestamp}">⬅️ Back to Summary</a>
...
generateSummaryPage() {
  ...
  const chartData = `<script src="https://cdn.jsdelivr.net/npm/chart.js?t=${timestamp}"></script>
    <script>
  ...
  const tableRows = this.results.map(r => {
    return `
      <tr>
        <td><a href="${r.id}.html?t=${timestamp}">${r.name}</a></td>
        <td>${r.statusEmoji}</td>
        <td>1</td>
